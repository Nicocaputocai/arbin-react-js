...
export const Stepper = ({steps, currentStep}) => {
  ...
  const displayStep = newStep.map((step, index) => {
    return (
      <div  key={index} className={index !== newStep.lenght -1 ?"w-full items-center flex" :"flex items-center"}>
        ...
        <div className={`flex-auto border-t-2 transiition duration-500 ease-in-out ${step.completed ? "border-green-600" : "border-gray-300"}`}>
            {/* Linea*/}
        </div>
      </div>
    );
  ...
  return (
    <div className="flex justify-between mx-4 p-4 flex items-center">
      {displayStep}
    </div>
  );
};
